...
#### Fit different statistical distributions to a vector of observations and estimate the maximum likelihood parameterf for each distribution
...
#### Modify to add in kernel density plot of data. This is an empirical curve that is fitted. It doesn't assume a particular probabiility distrbution but smooths out histogram shape.
````{r}
p1 <-  p1 +  geom_density(linetype="dotted",size=0.75)
print(p1)
````

### Get maximum likelihood paramters for normal
#### Fit normal distrbution to data and grab max likelihood estimators of the two parameters for normal (mean and variance)
````{r}
...
### Plot Beta Probability Desnisty
